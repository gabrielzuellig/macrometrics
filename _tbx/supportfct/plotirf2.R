plotirf2 <- function(irf1, irfbands1, irf2, irfbands2, printvars, printstate, poslegend='topright'){
  ...
  for (vv in 1:n){
    maxim <- max(c(irf1[,vv], irf2[,vv], lo1[,vv], lo2[,vv], up1[,vv], up2[,vv]))
    maxim <- maxim + 0.1*abs(maxim)
    minim <- min(c(irf1[,vv], irf2[,vv], lo1[,vv], lo2[,vv], up1[,vv], up2[,vv]))
    ...
    if (vv == n){
      legend(poslegend, legend=printstate, col=c('black', 'blue'), lwd=2, lty=c(1,2), box.lty=0)
    }
    box()
  }
