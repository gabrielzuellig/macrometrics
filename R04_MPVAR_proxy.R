...
u_p <- as.matrix(u_p[timeinreg[(p+1):length(timeinreg)]==1,1])
u_q <- as.matrix(u_q[timeinreg[(p+1):length(timeinreg)]==1,])
z <- as.matrix(VAR$z[timeinreg[(p+1):length(timeinreg)]==1,1])
# only use periods for which z is not NA 
nona <- !is.na(z)
u_p <- as.matrix(u_p[nona, ], nrow=sum(nona))
u_q <- u_q[nona,]
z <- cbind(1, as.matrix(z[nona, ], nrow=sum(nona)))
