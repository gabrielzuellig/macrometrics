function plotirf2(irf1, irfbands1, irf2, irfbands2, printvars, path, printstate, poslegend)
...
figure()
for vv = 1:n
    maxim = max([irf1(:,vv); irf2(:,vv); up1(:,vv); up2(:,vv)]);
    maxim = maxim + 0.1*abs(maxim);
    minim = min([irf1(:,vv); irf2(:,vv); lo1(:,vv); lo2(:,vv)]);
    ...
    if vv == 3 && ~isempty(printstate)
        legend([p1 p2], printstate, 'Location',poslegend,'FontSize',10)
        legend boxoff
    end
