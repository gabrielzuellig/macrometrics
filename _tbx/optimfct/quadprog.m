...
% 
%   [WSOUT,FVAL,EXITFLAG,...] = QUADPROG(H,f,A,b,Aeq,beq,LB,UB,WS) starts
%   quadprog from the data in the warm start object WS, using the options
...
%   subsequent solver call, quadprog can work faster.
%
%   See also LINPROG, LSQLIN, OPTIMWARMSTART
...
if (nargin >= 9 && isa(X0, 'optim.warmstart.QuadprogWarmStart'))
    useWarmStart = true;
    if ~isempty(options)
...
if isempty(options)
    ...
    elseif isstruct(options)
        ...
        error('optim:quadprog:InvalidOptions', ...
...
% Check that all data is real.  Warm start enforces real type.
if ~(isreal(H) && isreal(A) && isreal(Aeq) && isreal(f) && ...
     isreal(B) && isreal(Beq) && isreal(lb) && isreal(ub) && (useWarmStart || isreal(X0)))
...
switch output.algorithm
    ...
    otherwise % must be active-set because we did error checking earlier
        ...
        % If the output structure is requested, we must reconstruct the
        % Lagrange multipliers in the postsolve. Therefore, set
        % computeLambda to true if the output structure is requested.
        flags.computeLambda = computeFirstOrderOpt;
        flags.detailedExitMsg = detailedExitMsg;
        flags.verbosity = verbosity;
        flags.caller = 'quadprog';
        ...
        if useWarmStart
            ...
            % count.
            flags.isLeastSquaresUnconstrained = false;
            [X,fval,exitflag,output,lambda] = ...
